...
public class Placeholder extends View {
    ...
    private void init(AttributeSet attributeSet) {
        ...
        if (attributeSet != null) {
            TypedArray obtainStyledAttributes = getContext().obtainStyledAttributes(attributeSet, C0137R.styleable.ConstraintLayout_placeholder);
            int indexCount = obtainStyledAttributes.getIndexCount();
            for (int i = 0; i < indexCount; i++) {
                int index = obtainStyledAttributes.getIndex(i);
                if (index == C0137R.styleable.ConstraintLayout_placeholder_content) {
                    this.mContentId = obtainStyledAttributes.getResourceId(index, this.mContentId);
                } else if (index == C0137R.styleable.ConstraintLayout_placeholder_emptyVisibility) {
    ...
    public void onDraw(Canvas canvas) {
        if (isInEditMode()) {
            canvas.drawRGB(DummyPolicyIDType.zPolicy_SetShortCuts_SwitchSessionDown, DummyPolicyIDType.zPolicy_SetShortCuts_SwitchSessionDown, DummyPolicyIDType.zPolicy_SetShortCuts_SwitchSessionDown);
            Paint paint = new Paint();
            paint.setARGB(255, DummyPolicyIDType.zPolicy_SetShortCuts_Open_Invite_Window, DummyPolicyIDType.zPolicy_SetShortCuts_Open_Invite_Window, DummyPolicyIDType.zPolicy_SetShortCuts_Open_Invite_Window);
    ...
    public void updatePreLayout(ConstraintLayout constraintLayout) {
        ...
        if (view != null) {
            ((LayoutParams) view.getLayoutParams()).isInPlaceholder = true;
