...
class ImageViewUtils {
    ...
    static void startAnimateTransform(ImageView imageView) {
        if (VERSION.SDK_INT < 21) {
            ...
            imageView.setTag(C0397R.C0399id.save_scale_type, scaleType);
            if (scaleType == ScaleType.MATRIX) {
                imageView.setTag(C0397R.C0399id.save_image_matrix, imageView.getImageMatrix());
            } else {
        ...
        if (VERSION.SDK_INT < 21) {
            animator.addListener(new AnimatorListenerAdapter() {
                public void onAnimationEnd(Animator animator) {
                    ScaleType scaleType = (ScaleType) imageView.getTag(C0397R.C0399id.save_scale_type);
                    imageView.setScaleType(scaleType);
                    imageView.setTag(C0397R.C0399id.save_scale_type, null);
                    if (scaleType == ScaleType.MATRIX) {
                        ImageView imageView = imageView;
                        imageView.setImageMatrix((Matrix) imageView.getTag(C0397R.C0399id.save_image_matrix));
                        imageView.setTag(C0397R.C0399id.save_image_matrix, null);
                    }
