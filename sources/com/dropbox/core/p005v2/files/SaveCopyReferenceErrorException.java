package com.dropbox.core.p005v2.files;

import com.dropbox.core.DbxApiException;
import com.dropbox.core.LocalizedText;

/* renamed from: com.dropbox.core.v2.files.SaveCopyReferenceErrorException */
public class SaveCopyReferenceErrorException extends DbxApiException {
    private static final long serialVersionUID = 0;
    public final SaveCopyReferenceError errorValue;

    public SaveCopyReferenceErrorException(String str, String str2, LocalizedText localizedText, SaveCopyReferenceError saveCopyReferenceError) {
        super(str2, localizedText, buildMessage(str, localizedText, saveCopyReferenceError));
        if (saveCopyReferenceError != null) {
            this.errorValue = saveCopyReferenceError;
            return;
        }
        throw new NullPointerException("errorValue");
    }
}
