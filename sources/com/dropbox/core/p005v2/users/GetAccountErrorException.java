package com.dropbox.core.p005v2.users;

import com.dropbox.core.DbxApiException;
import com.dropbox.core.LocalizedText;

/* renamed from: com.dropbox.core.v2.users.GetAccountErrorException */
public class GetAccountErrorException extends DbxApiException {
    private static final long serialVersionUID = 0;
    public final GetAccountError errorValue;

    public GetAccountErrorException(String str, String str2, LocalizedText localizedText, GetAccountError getAccountError) {
        super(str2, localizedText, buildMessage(str, localizedText, getAccountError));
        if (getAccountError != null) {
            this.errorValue = getAccountError;
            return;
        }
        throw new NullPointerException("errorValue");
    }
}
