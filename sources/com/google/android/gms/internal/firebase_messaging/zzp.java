...
final class zzp {
    ...
    public final List<Throwable> zza(Throwable th, boolean z) {
        Reference poll = this.zzo.poll();
        while (poll != null) {
            this.zzn.remove(poll);
            poll = this.zzo.poll();
        }
        List<Throwable> list = (List) this.zzn.get(new zzo(th, null));
        if (list != null) {
            return list;
        }
        Vector vector = new Vector(2);
        List<Throwable> list2 = (List) this.zzn.putIfAbsent(new zzo(th, this.zzo), vector);
