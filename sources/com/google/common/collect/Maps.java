...
public final class Maps {
    ...
    static class FilteredEntryMap<K, V> extends AbstractFilteredMap<K, V> {
        ...
        class KeySet extends KeySet<K, V> {
            ...
            public boolean removeAll(Collection<?> collection) {
                return removeIf(Predicates.m87in(collection));
            }

            public boolean retainAll(Collection<?> collection) {
                return removeIf(Predicates.not(Predicates.m87in(collection)));
            }
    ...
    private static class FilteredEntryNavigableMap<K, V> extends AbstractNavigableMap<K, V> {
        ...
        public NavigableSet<K> navigableKeySet() {
            return new NavigableKeySet<K, V>(this) {
                public boolean removeAll(Collection<?> collection) {
                    return Iterators.removeIf(FilteredEntryNavigableMap.this.unfiltered.entrySet().iterator(), Predicates.and(FilteredEntryNavigableMap.this.entryPredicate, Maps.keyPredicateOnEntries(Predicates.m87in(collection))));
                }

                public boolean retainAll(Collection<?> collection) {
                    return Iterators.removeIf(FilteredEntryNavigableMap.this.unfiltered.entrySet().iterator(), Predicates.and(FilteredEntryNavigableMap.this.entryPredicate, Maps.keyPredicateOnEntries(Predicates.not(Predicates.m87in(collection)))));
                }
            };
        }
    ...
    /* access modifiers changed from: private */
    @Nullable
    public static <K, V> Entry<K, V> unmodifiableOrNull(@Nullable Entry<K, ? extends V> entry) {
        if (entry == null) {
            return null;
        }
