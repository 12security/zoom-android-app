...
public final class FlowableRepeatUntil<T> extends AbstractFlowableWithUpstream<T, T> {
    ...
    static final class RepeatSubscriber<T> extends AtomicInteger implements FlowableSubscriber<T> {
        ...
        /* renamed from: sa */
        final SubscriptionArbiter f395sa;
        ...
        RepeatSubscriber(Subscriber<? super T> subscriber, BooleanSupplier booleanSupplier, SubscriptionArbiter subscriptionArbiter, Publisher<? extends T> publisher) {
            ...
            this.f395sa = subscriptionArbiter;
            this.source = publisher;
            this.stop = booleanSupplier;
        }

        public void onSubscribe(Subscription subscription) {
            this.f395sa.setSubscription(subscription);
        }
        ...
        public void subscribeNext() {
            if (getAndIncrement() == 0) {
                int i = 1;
                while (!this.f395sa.isCancelled()) {
                    long j = this.produced;
                    if (j != 0) {
                        this.produced = 0;
                        this.f395sa.produced(j);
                    }
