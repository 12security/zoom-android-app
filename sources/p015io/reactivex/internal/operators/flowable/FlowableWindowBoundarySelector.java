...
import p015io.reactivex.processors.UnicastProcessor;
...
public final class FlowableWindowBoundarySelector<T, B, V> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
    ...
    static final class OperatorWindowBoundaryCloseSubscriber<T, V> extends DisposableSubscriber<V> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f405w;
    ...
    static final class WindowBoundaryMainSubscriber<T, B, V> extends QueueDrainSubscriber<T, Object, Flowable<T>> implements Subscription {
        ...
        /* renamed from: ws */
        final List<UnicastProcessor<T>> f406ws;
        ...
        public void onNext(T t) {
            if (!this.done) {
                if (fastEnter()) {
                    for (UnicastProcessor onNext : this.f406ws) {
                        onNext.onNext(t);
                    }
                    if (leave(-1) == 0) {
                        return;
        ...
        public void drainLoop() {
            ...
            List<UnicastProcessor<T>> list = this.f406ws;
            int i = 1;
            while (true) {
                boolean z = this.done;
                ...
                if (z && z2) {
                    ...
                    if (windowOperation.f407w != null) {
                        if (list.remove(windowOperation.f407w)) {
                            windowOperation.f407w.onComplete();
                            if (this.windows.decrementAndGet() == 0) {
                                dispose();
                                return;
                            }
        ...
        public void close(OperatorWindowBoundaryCloseSubscriber<T, V> operatorWindowBoundaryCloseSubscriber) {
            ...
            this.queue.offer(new WindowOperation(operatorWindowBoundaryCloseSubscriber.f405w, null));
            if (enter()) {
                drainLoop();
            }
        }
    ...
    static final class WindowOperation<T, B> {
        ...
        /* renamed from: w */
        final UnicastProcessor<T> f407w;

        WindowOperation(UnicastProcessor<T> unicastProcessor, B b) {
            this.f407w = unicastProcessor;
            this.open = b;
        }
    }
